    ...
    startTransition(async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/translateDocument`,
        {
          ...
          body: JSON.stringify({ documentData, targetLang: langauge }),
        }
      );
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        ...
        <form className="flex gap-2" onSubmit={handleAskQuestion}>
          <Select value={langauge} onValueChange={(e) => setLangauge(e)}>
            <SelectTrigger className="w-full">
              <SelectValue placeholder="Select a language" />
            </SelectTrigger>
            <SelectContent>
              {langauges.map((langauge) => (
                <SelectItem key={langauge} value={langauge}>
                  {langauge.charAt(0).toUpperCase() + langauge.slice(1)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
